...
(function($) {
	...
	* + Sticky menu
	* + Tootips
	*https://www.kcc.knowledgewing.com/ELDEV/
	/u/p:el_ABV10


	/*********************  snow animation on Start  **********************/
	...
	let showButton = function () {
		...
		return function () {
			if ($(this).scrollTop() > offset) {
				$(id_button).show();
				$(".text-animate p").css({color: "#00b9fe"});
			} else {
				$(id_button).hide();
				$(".text-animate p").css({color: "rgb(0, 255, 76)"});
			}
		}
	};
	...
	function loopText() {
		...
		$(".text-animate").css({left: widthScreen});
		$(".text-animate").css({"z-index": 2}).animate({
			left: 0
